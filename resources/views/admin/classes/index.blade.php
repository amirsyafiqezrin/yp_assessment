                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-black">
                            <thead class="text-xs text-black uppercase bg-gray-100">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 font-bold">Subjects Assigned</th>
                                    <th scope="col" class="px-6 py-3 font-bold">Students Count</th>
                            ...
                            <tbody>
                                @foreach($classes as $class)
                                    <tr class="bg-white border-b border-gray-200 hover:bg-gray-50 text-black">
                                            ...
                                            @if($class->subjects->count() > 0)
                                                <div class="flex flex-wrap gap-1">
                                                    @foreach($class->subjects as $subject)
                                                        <span
                                                            class="bg-indigo-100 text-indigo-900 border border-indigo-200 text-xs font-bold px-2 py-0.5 rounded">{{ $subject->code }}</span>
                                                    @endforeach
                                                </div>
                                            @else
                                        ...
                                        <td class="px-6 py-4 font-medium">{{ $class->students_count }}</td>
